...
contract StrategyLp {
    ...
    address constant public want = address(0xe67B96a9456E6e623463d022Fbb949a3fdD9Ed4C);
    address constant public pool = address(0x20eC291bB8459b6145317E7126532CE7EcE5056f);
     address constant public _pair = address(0xe67B96a9456E6e623463d022Fbb949a3fdD9Ed4C);
    address constant public output = address(0xE02dF9e3e622DeBdD69fb838bB799E3F168902c5);
    ...
    constructor() public {
        ...
        getName = string(
            abi.encodePacked("xyz:Strategy:", 
                abi.encodePacked(IERC20(want).name(),"Lp Token"
                )
            ));
    ...
    function deposit() public {
        ...
        if (_want > 0) {
            ...
            dRewards(pool).deposit(_pair, _want);
        }
    ...
    function _withdrawAll() internal {
        ...
        dRewards(pool).withdraw(_pair,user.amount);
    }
    
    function harvest() public {
        require(!Address.isContract(msg.sender),"!contract");
        dRewards(pool).deposit(_pair,0);
    ...
    function getNumOfRewards() public returns (uint256 pending) {
        pending = dRewards(pool).pendingBake(_pair,address(this));
    }

    function doswap() internal {
       
        uint256 _2yfam = IERC20(output).balanceOf(address(this));
        if(_2yfam > 1000)
        {
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint) {
        ...
        dRewards(pool).withdraw(_pair,_amount);
